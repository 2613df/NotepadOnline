...
if($_GET['api']==1){
	...
	if($_POST['es']!='e' and $_POST['es']!='s'){echo json_encode(array("status"=>"paraError3"));die();}
	if($_POST['name']=='' or !$_POST['name']){echo json_encode(array("status"=>"paraError4"));die();}
	if($_POST['do']=='' or !$_POST['do']){echo json_encode(array("status"=>"paraError5"));die();}
	$name = $_POST['name'];
	$es = $_POST['es'];
	$do = $_POST['do'];
	$data = $_POST['data'];
...
switch($do){
	...
	case 'noteChange':if($es=='s'){echo json_encode(array("status"=>"paraError7"));die();}echo $noteE->noteChange($data);break;
